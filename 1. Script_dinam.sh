...
PASSsv1="usuario"
PASSsv2="melon"
SVIP0=$1                   # IP inicial de la Server
SVIP1=192.168.237.2        # IP para Enp1s0 del Server
SVIP2=192.168.1.2          # IP estática que se aplicará en la Enp2s0 del Server
...
echo "$PASSsv1" | sudo -S bash -c '
   ...
   if [ ! -f /etc/netplan/50-cloud-init.yaml.bkup ]; then
    ...
    cat > /etc/netplan/50-cloud-init.yaml << EONET#!/bin/bash
# Script de configuración desde la máquina local hacia la máquina Server
